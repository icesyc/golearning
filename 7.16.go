...
func compute(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if err != nil {
			...
			exprValue = strconv.FormatFloat(ast.Eval(env), 'f', -1, 64)
		}
	}
...
<html>
		...
		<body>
			<form class="container" method="post" action="/">
				<div class="line">
					<input class="expr" type="text" name="expr" readonly value="{{.}}"/>
				</div>
				<div class="line">
		...
		<script type="text/javascript">
			...
			var expr = document.getElementsByClassName('expr')[0];
			for(var i = 0; i < buttons.length; i++){
				if(buttons[i].type == 'submit') continue;
				buttons[i].onclick = function(e){
					expr.value = expr.value + this.textContent
				}
			}
		</script>
	</html>
